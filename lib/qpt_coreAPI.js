...
var QingyingptAPI = module.exports = function(option){
	...
	this.addr = option.addr || 'pt.hit.edu.cn';
...
QingyingptAPI.prototype.doPostwithFile = function(path, fileName, fileContent, args, callback){
	...
	for(var key in args){
		data += '--' + boundary + '\r\n'
			...
			+ 'name="' + key + '"\r\n'
			+ '\r\n'
			+ new Buffer(new Buffer(args[key]).toString('utf8')).toString('binary') + '\r\n';
	}
	...
	data += '--' + boundary + '\r\n'
		...
		+ '\r\n'
		+ new Buffer(fileContent).toString('binary') + '\r\n';
...
QingyingptAPI.prototype.upload = function(filePath, options, callback){
	...
	fs.readFile(filePath, function(err, data){
		...
		self.doPostwithFile(self.parameters.portal['upload'], filePath.substring(filePath.lastIndexOf('/') + 1), data, args, callback);
	});
}
